...
**최신 프로젝트 백업 (Docker + Windows + TabPFN 지원)**: https://www.genspark.ai/api/files/s/FsZglohz
- 파일 크기: 1.4 MB
...
```bash
# 최신 버전 다운로드 (wget 사용)
wget https://www.genspark.ai/api/files/s/FsZglohz -O pci-api-docker.tar.gz

# 또는 curl 사용
curl -L https://www.genspark.ai/api/files/s/FsZglohz -o pci-api-docker.tar.gz
...
```bash
# 최신 버전 다운로드 (wget 사용)
wget https://www.genspark.ai/api/files/s/FsZglohz -O pci-api.tar.gz

# 또는 curl 사용
curl -L https://www.genspark.ai/api/files/s/FsZglohz -o pci-api.tar.gz
